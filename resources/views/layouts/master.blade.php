                    ...
                    <li class="dropdown notification-list list-inline-item">
                        <div class="dropdown notification-list nav-pro-img">
                            ...
                            <div class="dropdown-menu dropdown-menu-right dropdown-menu-animated profile-dropdown">
                                ...
                                <a class="dropdown-item" href="{{ route('admin/profile') }}"><i class="mdi mdi-account-circle" style="color: #2c3749"></i>{{ auth()->user()->name }}</a>
   ...
   <script>
        function exportReportToExcel(filename) {
            ...
            TableToExcel.convert(table[0], { // html code may contain multiple tables so here we are refering to 1st table tag
                name: filename+` Task & Report.xlsx`, // fileName you could use any name
                sheet: {
    ...
    $(document).on('click','.taskUpdate',function()
     {
         ...
         $('#e_billing').val(_this.find('.billing').text());
         $('#e_billed').val(_this.find('.billed').text());
         $('#e_notes').val(_this.find('.notes').text());
         // $('#e_image').val(_this.find('.image').text());

         var user_id = (_this.find(".user_id").text());
         var _option = '<option selected value="' + user_id+ '">' + _this.find('.user').text() + '</option>'
         $( _option).appendTo("#e_user_id");
    ...
    $(document).on('click','.projectUpdate',function()
     {
        ...
        //  $( _option).appendTo("#e_client");

        //  var user = (_this.find(".user").text());
    ...
    $(document).on('click','.userUpdate',function()
     {
        ...
        //  var _option = '<option selected value="' + role_name+ '">' + _this.find('.role_name').text() + '</option>'
